...
class HotelItemCallbackHandler:
    ...
    def _get_item_instance_by_route_key(self, route_key):
        if route_key == 'room':
            self._get_action_method(self.method, Room, RoomSerializer)
        if route_key == 'hotel':
            self._get_action_method(self.method, Hotel, HotelSerializer)
        if route_key == 'hotel_option':
            self._get_action_method(self.method, HotelOptions, HotelOptionsSerializer)
